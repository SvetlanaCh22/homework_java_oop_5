...
public abstract class Archers extends Hero {
    ...
    private Hero getTarget(ArrayList<Hero> heroList) {
        ...
        for (int i = 1; i < heroList.size(); i++) {
            ...
            System.out.println("Distance from "+getRole()+" "+getName()+" to "+heroObj.getRole()+" "+heroObj.getName()+" is "+getPosition().getDistance(heroObj.getPosition().x, heroObj.getPosition().y));
        }
        System.out.println("[TARGET] Distance from "+getRole()+" "+getName()+" to "+heroObj.getRole()+" "+heroObj.getName()+" is "+getPosition().getDistance(heroTarget.getPosition().x, heroTarget.getPosition().y));
        return heroTarget;
    }
